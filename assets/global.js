...
class VariantSelects extends HTMLElement {
  ...
  static calculatePrice() {
    const price = document.querySelector(".js-price"),
          ...
          btnText = document.querySelector(".product-form__btn-price");

    let   quantityInputValue = quantityInput?.value,
          priceValue = price?.textContent,
          btnTextContent = +quantityInputValue * +priceValue,
          mainPriceValue,
          subPriceValue;
      
    if (btnText) {
      if (quantityInputValue > 0) {
        mainPriceValue = btnTextContent.toString().split('.')[0],
        subPriceValue = btnTextContent.toString().split('.')[1].slice(0,2);
...
document.onload = VariantSelects.calculatePrice();
...
function checkHeaderPosition() {
  ...
  if (headerPos.top < 0) {
    header.classList.add("header-offset-top");
  } else {
    header.classList.remove("header-offset-top");
  }
}

checkHeaderPosition();

function moveElement(elem, wrapper) {
  let element = document.querySelector(elem);
  let moveTo = document.querySelector(wrapper);
  let parentElement = element?.parentElement;

  addEventListener("resize", () => {
    if (window.matchMedia("(max-width: 749px)").matches) {
      moveTo?.prepend(element);
    } else {
...
moveElement(".product-text__wrap-content", ".js-product" );
moveElement(".product-variant__wrap", ".product__info-wrapper");
